...
func select_launcher_source(goal_index: int) -> bool:
	...
	# Start and completed checkpoints own their generated leg's protected departure
	...
	if goal_index < 0:
		launcher.shot_axis_yaw_degrees = prepared_course.legs[0].shot_axis_yaw_degrees
	elif goal_index + 1 < prepared_course.legs.size():
