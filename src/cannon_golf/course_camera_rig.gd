...
const CANNON_BACK_DISTANCE := 80.0
const CANNON_SIDE_DISTANCE := 12.0
const CANNON_HEIGHT := 45.0
...
func set_cannon_pose(
	...
	if _return_snapshot.get("view", &"") == &"cannon":
		_return_snapshot["pan"] = Vector3.ZERO
		_return_snapshot["zoom"] = DEFAULT_ZOOM
		_return_snapshot["orbit"] = Vector2.ZERO
	_pose_dirty = true
	return true
...
func pan(screen_direction: Vector2) -> void:
	...
	pan_offset += right * screen_direction.x * scale + forward * screen_direction.y * scale
	_clamp_pan()
	_pose_dirty = true
...
func planning_focus() -> Vector3:
	if view_mode == &"cannon":
		return _terrain_safe_focus(_cannon_anchor + pan_offset)
...
func _resolve_pose(size: Vector2) -> void:
	if view_mode == &"cannon":
		...
		var offset := base_position - _cannon_anchor
		offset = offset.rotated(Vector3.UP, deg_to_rad(orbit_degrees.x))
		var orbit_right := Vector3.UP.cross(offset).normalized()
		if not orbit_right.is_zero_approx():
			offset = offset.rotated(orbit_right, deg_to_rad(-orbit_degrees.y))
		offset = offset.normalized() * _resolved_cannon_distance(offset.length())
...
func _clamp_pan() -> void:
	var base_focus := _cannon_anchor if view_mode == &"cannon" else _base_focus
	...
	pan_offset = Vector3(focus.x - base_focus.x, 0.0, focus.z - base_focus.z)


func _resolved_cannon_distance(base_distance: float) -> float:
	if zoom >= DEFAULT_ZOOM:
		return exp(lerpf(
			log(base_distance), log(CLOSE_INSPECTION_DISTANCE),
			clampf(zoom / CLOSE_ZOOM, 0.0, 1.0)
		))
	var course_diagonal := Vector2(_frame_bounds.size.x, _frame_bounds.size.z).length()
	var whole_course_distance := maxf(base_distance, course_diagonal * 1.18)
	return exp(lerpf(
		log(base_distance), log(whole_course_distance),
		clampf(-zoom / -OVERVIEW_ZOOM, 0.0, 1.0)
	))
