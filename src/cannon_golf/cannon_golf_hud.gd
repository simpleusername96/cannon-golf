...
func apply_language(language: String) -> void:
	...
	%DragKey.text = "L Drag" if english else "좌 드래그"
	%ShortcutDrag.text = "Move view" if english else "화면 이동"
	%OrbitKey.text = "R / Shift+L" if english else "우 / Shift+좌"
	%ShortcutOrbit.text = "Orbit view" if english else "화면 회전"
