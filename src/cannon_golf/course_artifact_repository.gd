...
const MAX_CACHED_COURSES := 15

var _active_request: Dictionary = {}
var _queued_request: Dictionary = {}
var _prefetch_queue: Array[CannonGolfCourseData] = []
...
## Warms immutable prepared artifacts while explicit selection keeps priority.
func prefetch_courses(courses: Array[CannonGolfCourseData]) -> void:
	for course in courses:
		if course == null or ready_course(course) != null \
				or _request_matches(_active_request, course, CannonGolfCourseCatalog.prepared_path_for(course)) \
				or _request_matches(_queued_request, course, CannonGolfCourseCatalog.prepared_path_for(course)) \
				or _prefetch_queue.has(course):
			continue
		_prefetch_queue.append(course)
...
func _start_prefetch_request() -> void:
	if not _active_request.is_empty() or not _queued_request.is_empty():
		return
	while not _prefetch_queue.is_empty():
		var course: CannonGolfCourseData = _prefetch_queue.pop_front()
		if course == null or ready_course(course) != null:
			continue
		var path := CannonGolfCourseCatalog.prepared_path_for(course)
		if path.is_empty():
			continue
		_start_request({"course": course, "path": path})
		return
...
func _start_request(request: Dictionary) -> void:
	...
	if error != OK:
		if course != null and course.course_id == _desired_course_id:
			course_failed.emit(course.course_id)
		return
